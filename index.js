...
// Objects
const geometry = new THREE.SphereGeometry(.2, .09,  16, 100)
...
targetY = mouseY *.01
 ...
 if(flag == 0) 
    // Update objects
    if(mouseX> 0)
    if (model) model.rotation.y += -mouseY * (deltaTime *0.00009)
    if (model) model.rotation.y += -mouseX * (deltaTime *0.00009)
